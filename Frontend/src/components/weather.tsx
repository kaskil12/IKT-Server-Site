...
export function Weather() {
  ...
  useEffect(() => {
    fetch(
      "https://api.met.no/weatherapi/locationforecast/2.0/compact?lat=60.88&lon=11.56"
    )
  ...
  return (
    <Card
      className={`w-full max-w-md mx-auto bg-gradient-to-r ${getColorClass(
        color
      )} ${TextColor(color)} p-4 rounded-2xl shadow-xl`}
    >
      <CardHeader>
        ...
        <p className="text-center text-sm">
          {data.properties?.timeseries[0]?.time}
        </p>
      </CardHeader>
      <CardContent className="grid grid-cols-2 gap-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <p>
            {weather.wind_speed} m/s ({Math.round(weather.wind_from_direction)}
            °)
          </p>
        </div>
      </CardContent>
