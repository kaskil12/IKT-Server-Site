...
function SwitchCardItem({ switcher, history, onDelete, onUpdate }: { switcher: Switcher; history: any[]; onDelete: (id: number) => Promise<void>; onUpdate: (id: number, updates: any) => Promise<void>; }) {
                ...
                <div className="flex gap-2">
                    {editing ? (
                        <button className="bg-green-600 text-white px-3 py-1 rounded-lg" onClick={async () => {
                            await onUpdate(switcher.id, local);
                            setEditing(false);
                        }}>Save</button>
                    ) : (
                        <button className="bg-blue-600 text-white px-3 py-1 rounded-lg" onClick={() => setEditing(true)}>Edit</button>
                    )}
                    <button className="bg-red-600 text-white px-3 py-1 rounded-lg float-right" onClick={async () => { await onDelete(switcher.id); }}>Slett</button>
                </div>
            </div>
                ...
                <div className="flex-1">
                    <AreaChart width={350} height={180} data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                        ...
                        <XAxis dataKey="date" tick={{ fill: "#fff" }} tickLine={false} axisLine={false} minTickGap={24}
                            tickFormatter={(value) => {
                                try { const d = new Date(value); return d.toLocaleString(); } catch { return String(value); }
                            }}
                        />
                        <Tooltip />
                        <Legend />
    ...
    return (
        <>
            {Array.isArray(switches) && switches.map((switcher) => {
                ...
                return <SwitchCardItem key={switcher.id} switcher={switcher} history={history} onDelete={deleteSwitcher} onUpdate={updateSwitcher} />;
            })}
        </>
    );
}